...
sf::Vector2f Map::GetVector(sf::Vector2<double> player_pos, sf::Vector2i mouse_pos) {
    sf::Vector2f dir;
    ...
    case sf::Event::MouseButtonPressed:
        if (event.key.code == sf::Mouse::Left) {
           /* sf::Vector2i mouse = sf::Mouse::getPosition(*window);
            ammo.push_back(Ammo(sf::Vector2<double>(0.0, 0.0), 0));*/
        }
        break;
    default:
        break;
...
void Map::CollisionListener(double time) {
    for (std::size_t i = 0; i < ammo.size(); i++) {
        ammo[i].Move(sf::Vector2<double>(0, 0), time);
    }
